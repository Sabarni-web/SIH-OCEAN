import type { CurrentVectorPoint, TimelineFrame, TimelineResponse, SSTGridPoint, InstrumentSnapshot } from '../../../shared/types';
import { fetchHistoricalArgoPositions, buildInstrumentSnapshots } from './timeline-instruments.service';
...
interface CacheEntry {
  timestamp: number;
  data: CurrentVectorPoint[];
}

const cache = new Map<string, CacheEntry>();
const CACHE_TTL_MS = 15 * 60 * 1000; // 15 minutes

export const fetchLiveOceanCurrents = async (dateStr?: string): Promise<CurrentVectorPoint[]> => {
  ...
  try {
    ...
    const results: CurrentVectorPoint[] = pointList.map((item, idx) => {
    ...
    const fallbackResults: CurrentVectorPoint[] = GRID_COORDINATES.map(coord => {
...
const formatTimelineLabel = (isoStr: string): string => {
  try {
  ...
  try {
    ...
    const [openMeteoResponse, argoHistory] = await Promise.all([
      (async () => {
        ...
        clearTimeout(timeout);
        if (!res.ok) throw new Error(`Open-Meteo API returned HTTP status ${res.status}`);
        return res.json();
      })(),
      fetchHistoricalArgoPositions(cleanStart, cleanEnd)
    ]);
    ...
    const timeArray: string[] = pointList[0].hourly.time;
    ...
    for (let h = 0; h < totalHours; h += stepHours) {
      ...
      const sst: SSTGridPoint[] = pointList.map((item, idx) => {
        const lat = item.latitude ?? GRID_COORDINATES[idx]?.lat ?? 0;
        const lon = item.longitude ?? GRID_COORDINATES[idx]?.lon ?? 0;
      ...
      if (frames.length >= 360) break;
    }

    const frameTimestamps = frames.map(f => f.timestamp);
    const instrumentSnapshots = buildInstrumentSnapshots(frameTimestamps, argoHistory);
    ...
    const fallbackFrames: TimelineFrame[] = [];
    ...
    for (let i = 0; i < sampleCount; i++) {
      ...
      const vectors: CurrentVectorPoint[] = GRID_COORDINATES.map(coord => {
        const latRad = (coord.lat * Math.PI) / 180;
        const lonRad = (coord.lon * Math.PI) / 180;
        const isSomaliJet = coord.lon < 58 && coord.lat > 0 && coord.lat < 12;
      ...
      const sst: SSTGridPoint[] = GRID_COORDINATES.map(coord => ({
        latitude: coord.lat,
        longitude: coord.lon,
    ...
    const fallbackSnapshots = buildInstrumentSnapshots(
      fallbackFrames.map(f => f.timestamp),
      new Map()
    );
