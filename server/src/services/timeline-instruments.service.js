...
exports.fetchHistoricalArgoPositions = fetchHistoricalArgoPositions;
exports.buildInstrumentSnapshots = buildInstrumentSnapshots;
const incois_service_1 = require("./incois.service");
async function fetchHistoricalArgoPositions(startDate, endDate) {
    const baseUrl = (0, incois_service_1.getErddapBaseUrl)();
    const datasetId = (0, incois_service_1.getArgoDatasetId)();
    ...
    const byWmo = new Map();
    try {
        const response = await (0, incois_service_1.fetchJson)(url);
    ...
    if (byWmo.size === 0) {
        try {
            const liveFloats = await (0, incois_service_1.fetchLiveIncoisArgoFloats)('30d');
...
function buildInstrumentSnapshots(frameTimestamps, argoHistory) {
    return frameTimestamps.map((frameTs) => {
        ...
        const activeGliderIndices = [];
        incois_service_1.INCOIS_GLIDERS.forEach((glider, idx) => {
            if (!glider.track || glider.track.length === 0)
                return;
        ...
        const activeCTDIds = incois_service_1.INCOIS_CTD_STATIONS
        ...
        const activeBGCIds = incois_service_1.INCOIS_BGC_FLOATS
...
//# sourceMappingURL=timeline-instruments.service.js.map