...
function createLandShape(points: [number, number][]): THREE.Shape {
      ...
      {[50, 70, 90, 105].map((lon) => {
        ...
        return (
          <Html key={`lon-${lon}`} position={[x, 0.1, SCENE_DIMENSIONS.depth / 2 + 0.8]} center className="pointer-events-none select-none">
            <span className="text-[9px] font-mono text-cyan-400/70">{lon}°E</span>
          </Html>
        );
      })}
    </group>
